...
from src.utils import load_obj_tsv, load_spatial_data, load_spatial_gqa, load_patches
...
Split2VidFeatPath = {
    ...
    # 'mscoco_nominival': '/data/Grounded-RL2021/lxmert/data/mscoco_imgfeat/valid_spatial.tsv.tsv',
    'vgnococo': 'data/gqa/gqa_spatial.h5',
}

Split2VidPatchesPath = {
...
class LXMERTTorchDataset(Dataset):
    def __init__(self, dataset: LXMERTDataset, topk=-1):
        ...
        for source in self.raw_dataset.sources:
            if source == 'vgnococo':
                img_data.extend(load_spatial_gqa(Split2VidFeatPath[source], topk))
            else:
                img_data.extend(load_spatial_data(Split2VidFeatPath[source], topk))
...
class LXMERTPatchesTorchDataset(Dataset):
    def __init__(self, dataset: LXMERTDataset, topk=-1):
        ...
        for source in self.raw_dataset.sources:
            if source == 'vgnococo':
                img_data.extend(load_patches(Split2VidPatchesPath[source], dataset='gqa', topk=topk))
            else:
                img_data.extend(load_patches(Split2VidPatchesPath[source], dataset='MSCOCO', topk=topk))
