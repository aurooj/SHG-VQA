...
class PrimaryCaps(nn.Module):
    ...
    def __init__(self, A, B, K, P, stride):
        ...
        self.pose = nn.Conv3d(in_channels=A, out_channels=B * P * P,
                              kernel_size=K, stride=stride, bias=True)
        self.pose.weight.data.normal_(0.0, 0.1)
        self.a = nn.Conv3d(in_channels=A, out_channels=B,
                           kernel_size=K, stride=stride, bias=True)
        self.a.weight.data.normal_(0.0, 0.1)
        self.sigmoid = nn.Sigmoid()
...
class ConvCaps(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.w_shared:
            ...
            p_out = p_out.view(b, oh, ow, self.C, self.psize) #changed code from=> p_out.view(b, oh, ow, self.C *self.psize)
