...
# from ovr_cnn.maskrcnn_benchmark.modeling.mmss_heads import GroundingHead
...
class HGEmbeddings(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings.
    """
    def __init__(self, num_queries, type_vocab_size, hidden_size, hidden_dropout_prob=0.1, gt_hg=False):
        super(HGEmbeddings, self).__init__()
        self.gt_hg = gt_hg
        self.word_embeddings = nn.Embedding(num_queries, hidden_size, padding_idx=0)
        self.token_type_embeddings = nn.Embedding(type_vocab_size, hidden_size, padding_idx=0)
    ...
    def forward(self, token_type_ids=None, token_ids=None):
        B = token_type_ids.size(0)
        #todo: for GT HG input, we use embedding layer with embeddings equal to num_classes+1, and get word_embeddings based on the word_ids
        if self.gt_hg and token_ids is not None:
            words_embeddings = self.word_embeddings(token_ids)
        else:
            words_embeddings = self.word_embeddings.weight.unsqueeze(0).repeat(B, 1, 1)
...
class SelfCrossLayer(nn.Module):
    ...
    def self_att(self, lang_input, lang_attention_mask, visn_input, visn_attention_mask, step):
        ...
        if step == 0:
            ...
            att_output, attention_probs = self.cross_att(vis_lang_input, visn_attention_mask)
...
class VisualFeatEncoder(nn.Module):
    def __init__(self, config, norm_inputs=False, patches=False, no_caps=False):
        ...
        if self.no_caps:
            ...
            if self.is_attn_routing:
                ...
                self.primary_caps = PrimaryCaps(config.hidden_size, num_prim_caps, (1, 1, 1), P, stride=1)
    ...
    def forward(self, visn_input):
        ...
        if self.no_caps:
            _, c, t, H, W = img.shape
            out = img.permute(0, 2, 3, 4, 1).view(-1, t * H * W, c)
...
class CrossEncoder(nn.Module):
    def __init__(self, config, cross_attn_type= 'cross', mask_features=False, num_max_act=3, num_max_rel=8, add_action=False, add_relation=False):
        super().__init__()
        # self.no_caps = no_caps
        self.config = config
        ...
        self.num_max_act = num_max_act
        self.num_max_rel = num_max_rel

        self.add_action = add_action
        self.add_relation = add_relation

        self.action_only = add_action and not add_relation
        self.relation_only = add_relation and not add_action
        self.both = add_action and add_relation if hasattr(self, "both" ) else (add_action and add_relation)

        if self.action_only:
            self.hg_len = self.num_max_act
        if self.relation_only:
            self.hg_len = self.num_max_rel
        if self.both:
            self.hg_len = self.num_max_act + self.num_max_rel


        #special tokens for action and relations
        self.act_token = nn.Parameter(torch.zeros(1, 1, config.hidden_size))
        self.rel_token = nn.Parameter(torch.zeros(1, 1, config.hidden_size))
        self.cls_token = nn.Parameter(torch.zeros(1, 1, config.hidden_size))
        ...
        print("Cross encoder with %d x_layers." %
              (self.num_x_layers))
    ...
    def forward(self, lang_feats, lang_attention_mask,
                hg_feats, hg_attention_mask=None, output_all_attention_masks=False):
        ...
        B, T, D = hg_feats.shape
        T = T // self.hg_len if hasattr(self, "hg_len") else T // (self.num_max_act + self.num_max_rel)

        #create hg type embedding
        act_tokens = self.act_token.expand(B, self.num_max_act, -1)
        rel_tokens = self.rel_token.expand(B, self.num_max_rel, -1)

        if self.action_only:
            hg_type_tokens = act_tokens
        elif self.relation_only:
            hg_type_tokens = rel_tokens
        elif hasattr(self, "both") and self.both:
            hg_type_tokens = torch.cat([act_tokens, rel_tokens], dim=1)

        else:
            hg_type_tokens = torch.cat([act_tokens, rel_tokens], dim=1)

        hg_feats = (hg_feats.view(B, T, -1, D) + hg_type_tokens.unsqueeze(1)).view(B, -1, D)

        cls_tokens = self.cls_token.expand(B, -1, -1)
        hg_feats = torch.cat((cls_tokens, hg_feats), dim=1)

        # Process the hg attention mask
        if hg_attention_mask is not None:
            hg_attention_mask = torch.cat((torch.ones(B, 1).to(device=hg_attention_mask.device), hg_attention_mask.view(B, -1)), dim=1)
            extended_hg_attention_mask = hg_attention_mask.unsqueeze(1).unsqueeze(2)
            extended_hg_attention_mask = extended_hg_attention_mask.to(
                dtype=lang_attention_mask.dtype)  # fp16 compatibility
            extended_hg_attention_mask = (1.0 - extended_hg_attention_mask) * -10000.0
        else:
            extended_hg_attention_mask = None
        ...
        for layer_module in self.x_layers:
            lang_feats, hg_feats, attn_probs = layer_module(lang_feats, lang_attention_mask,
                                                  hg_feats, extended_hg_attention_mask)
        ...
        pooled_output = self.pooler(hg_feats, lang_feats)

        return pooled_output, (all_attention_mask_t,
                                        all_attnetion_mask_v,
                                        all_attention_mask_xl,
                                        all_attention_mask_xv,
                                        all_attention_mask_vl)
...
class NoCapsEncoder(nn.Module):
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        return lang_feats, visn_feats, (all_attention_mask_t,
                                        ...
                                        all_attention_mask_xv,
                                        all_attention_mask_vl,
                                        (lang_feats_out, lang_attention_mask, visn_feats_out, visn_attention_mask))
...
class BertNoCapsEncoder(nn.Module):
    def __init__(self, config, shared_weights=False, cross_attn=False, cross_attn_type= 'self', no_caps=False, mask_features=False):
        super().__init__()
        self.no_caps = no_caps
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        return lang_feats_out, lang_attention_mask





class BertCapsulesEncoder(nn.Module):
    def __init__(self, config,
                 shared_weights=False,
                 cross_attn=False,
                 cross_attn_type='self',
                 freeze_weights=False,
                 skip_connection=True,
                 patches=False,
                 vit_init=False,
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        return lang_feats, all_attention_mask_t




class BertNoCapsModel(BertPreTrainedModel):
    """LXRT Model."""


    def __init__(self, config, shared_weights=False, cross_attn=False, cross_attn_type='self', no_caps=False):
        super().__init__(config)
        self.embeddings = BertEmbeddings(config)
        self.encoder = BertNoCapsEncoder(config,no_caps=no_caps)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None):
        ...
        # Run LXRT backbone
        lang_feats, attn_probs = self.encoder(
            embedding_output,
            extended_attention_mask,
            visn_feats = None)


        return lang_feats, lang_feats[:,0], attn_probs
...
class BertCapsuleModel(BertPreTrainedModel):
    """LXRT Model."""
    ...
    def __init__(self, config,
        ...
        super().__init__(config)
        self.embeddings = BertEmbeddings(config)
        self.encoder = BertCapsulesEncoder(config,
                                           shared_weights=shared_weights,
                                           cross_attn=cross_attn,
                                           cross_attn_type=cross_attn_type,
                                           skip_connection=skip_connection,
...
class BertFeatureExtraction(BertPreTrainedModel):
    ...
    def __init__(self, config,
        ...
        #if skip_connection:
        if no_caps:
            self.bert = BertNoCapsModel(config, no_caps=no_caps)
        else:
            self.bert = BertCapsuleModel(config,
                                         shared_weights=shared_weights,
                                         skip_connection=skip_connection,
                                         patches=patches,
                                         vit_init=vit_init,
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,
                visual_attention_mask=None, output_all_attention_masks=False,):
        feat_seq, output, attn_probs = self.bert(input_ids, token_type_ids, attention_mask)
...
"""DEAF MODEL CLASSES"""
class DeafNoCapsEncoder(nn.Module):
    def __init__(self, config, shared_weights=False, cross_attn=False, cross_attn_type='self', no_caps=False, mask_features=False):
        super().__init__()
        self.no_caps = no_caps
        self.mask_features = mask_features
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        for layer_module in self.x_layers:
            lang_feats, visn_feats, attn_probs = layer_module(lang_feats, lang_attention_mask,
                                                  visn_feats, visn_attention_mask, step=1)      #todo: step=1 for one modality
            if output_all_attention_masks:
                all_attention_mask_t.append(attn_probs['attn_prob_l'])
                all_attnetion_mask_v.append(attn_probs['attn_prob_v'])
                all_attention_mask_xl.append(attn_probs['attn_prob_xl'])
...
class DeafNoCapsModel(BertPreTrainedModel):
    def __init__(self, config, shared_weights=False, cross_attn=False, cross_attn_type='self', no_caps=False):
        super().__init__(config)
        self.cross_attn_type = cross_attn_type
        self.embeddings = BertEmbeddings(config)
        self.encoder = DeafNoCapsEncoder(config,
                                     shared_weights=shared_weights,
                                     cross_attn=cross_attn,
                                     cross_attn_type=cross_attn_type,
                                     no_caps=no_caps)
...
class DeafFeatureExtraction(BertPreTrainedModel):
    """
    BERT model for classification.
    """
    def __init__(self, config,
        ...
        #if skip_connection:
        if no_caps:
            self.bert = DeafNoCapsModel(config,
                                    cross_attn_type=cross_attn_type,
                                    no_caps=no_caps
                                    )
        else:
